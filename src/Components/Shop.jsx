...
import shopCart7 from '../assets/shop-imgs/shop-cart7.jpg'
...
function Shop() {
  ...
  { id: 6, name: "engine", image: shopCart6 },
  { id: 7, name: "engine $ drivetrain", image: shopCart7 },
  { id: 8, name: "engine $ drivetrain", image: shopCart7 },
];


  return (
    <div className='flex flex-col p-2 px-4 mb-4 text-zinc-600 md:relative'>
          ...
          {shopCategories.map(item => {
            ...
            return (
              <div 
                ...
                className={`flex flex-col items-center justify-between border gap-2 capitalize mb-4 h-70 rounded-md ${isActive ? 'border-red-600' : 'border-gray-300'}`}
              >
                <div className='w-full h-full flex items-center justify-center'>
                  <img className='w-1/2 md:w-1/4' src={item.image} alt="shop-cart-imgs" />
                </div>
                <p className={`text-xl font-medium w-full text-center p-5 ${isActive ? 'bg-red-600 text-white' : 'bg-gray-200'}`}>
                  {item.name}
                </p>
              </div>
            );
      ...
      <button className="mt-6 bg-red-600 hover:bg-red-700 text-white p-4 px-8 md:mt-0 rounded-md  md:absolute md:right-4 md:top-0">
        Shop Now
      </button>
</div>
